...
export interface CanvasEditorProps {
  ...
  // React19: avoid forwardRef; expose API via explicit ref prop or callback
  editorRef?:
    | React.RefObject<CanvasEditorRef | null>
    | ((api: CanvasEditorRef | null) => void)
}
...
export const CanvasEditor = ({
  ...
  // expose APIs without forwardRef: use explicit prop editorRef
  const api = useMemo<CanvasEditorRef>(
    () => ({
      getMaskBase64: () => {
  ...
  useEffect(() => {
    ...
    ;(editorRef as React.RefObject<CanvasEditorRef | null>).current = api
    return () => {
      ;(editorRef as React.RefObject<CanvasEditorRef | null>).current = null
    }
  }, [api, editorRef])
  ...
  return (
    ...
    >
      ...
      >
        ...
        <button
          onClick={() =>
            setZoomPercent((z) => clamp(z - 10, minZoomPercent, maxZoomPercent))
          }
          style={{ padding: '6px 10px', background: '#fff', borderRadius: 8 }}
        >
          -
