...
const MCPSwitch = memo(
  function MCPSwitch(props: {
    mcpServers: ChatInputType['mcpServers']
    setMcpServers: (mcpServers: ChatInputType['mcpServers']) => void
  }) {
    const { mcpServers, setMcpServers } = props
    return (
      <div>
        <span>测试MCP: </span>
        <Switch
          className='flex-0'
          checked={!!mcpServers}
          onChange={(checked) => setMcpServers(checked ? [] : undefined)}
        />
      </div>
    )
  },
  (prev, next) => prev.mcpServers === next.mcpServers,
)
...
export function useInputPanel(options: { isSupportVision?: boolean }) {
  ...
  useEffect(() => {
    if (isSupportVision) {
      setQuestion('what is this image?')
    } else {
      setQuestion('What is the weather in 34.0522° N, -118.2437° W')
    }
  }, [isSupportVision])
  ...
  return {
    render: () => (
      <div className='flex flex-col gap-2 bg-blue-50 p-4 rounded-lg'>
        ...
        <MCPSwitch mcpServers={mcpServers} setMcpServers={setMcpServers} />
      </div>
    ),
    getter: {
      imageUrl,
